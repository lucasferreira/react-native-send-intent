...
public class RNSendIntentModule extends ReactContextBaseJavaModule {
	...
	public void sendMail(String recepientString, String subject, String body) {
		String uriText = "mailto:" + Uri.encode(recepientString) + "?body=" + Uri.encode(body) + "&subject="
				+ Uri.encode(subject);
		...
		new OkHttpClient().newCall(request).enqueue(new okhttp3.Callback() {
			...
			public void onResponse(final Call call, final Response response) {
				...
				try (final ResponseBody body = response.body()) {
					...
					final Intent intent = new Intent(Intent.ACTION_VIEW).setDataAndType(Uri.fromFile(file),
